...
def save_fig(fig, outfile, **kwargs):
    ...
    dpi = kwargs['dpi']
    print(f'Save to file (EXT={ext}, DPI={dpi}, W/H={w*dpi}/{h*dpi} p): {outfile}')
...
def read_img(fname, mask=None, dset='velocity', scale=None):
    """
    fname   e.g., velocity.h5, ifgramStack.h5
    mask    e.g., waterMask.h5, maskTempCoh.h5
    dset    e.g., velocity, unwrapPhase-yyyymmdd
    scale   convert unit from m to mm for velocity file
    """
    if mask in ['no', 'No', 'NO']:  mask = None

    if not scale:
        if dset == 'velocity':  scale = 1e3
        else:                   scale = 1.

    # read value from the file
    v     = readfile.read(fname, datasetName=dset)[0] * scale

    # read attributes and others
    if dset == 'velocity':
        atr  = readfile.read(fname, datasetName=dset)[1]
        try:
            v_std = readfile.read(fname, datasetName='{}Std'.format(dset))[0] * scale
        except:
            pass
...
def colorbar_inset(fig, ax, data, im, atr=None, cbar_inset=True, **kwargs):
    ...
    if cbar_inset:
        cbox = ax.inset_axes(kwargs['box_loc'], zorder=5)
...
def scalebar_inset(fig, ax, inps, **kwargs):
    ...
    if 'box_loc'        not in kwargs.keys():
        ...
        elif kwargs['loc'] == 'lower left':     kwargs['box_loc']       = (0, 0, .4, .06)
    if 'box_alpha'      not in kwargs.keys():   kwargs['box_alpha']     = .9
...
def satellite_inset(fig, ax, atr, **kwargs):
    ...
    if 'box_loc'        not in kwargs.keys():
        if atr == None:
            ...
            if   atr['ORBIT_DIRECTION'].startswith('A'):
                                            kwargs['box_loc']       = (0, .06, .15, .13)
            elif atr['ORBIT_DIRECTION'].startswith('D'):
                if kwargs['sat_corner'] is True:
                                            kwargs['box_loc']       = (1-.15, 0, .15, .13)
                elif kwargs['sat_corner'] == 'upperright':
                                            kwargs['box_loc']       = (1-.15, 1-.13, .15, .13)
                else:
                                            kwargs['box_loc']       = (1-.15, .3, .15, .13)

    sbox = ax.inset_axes(kwargs['box_loc'], zorder=5)
...
def plot_imgs(v, atr, dem, super_title=None, outfile=False, **kwargs):
    ...
    for i, (ax, k, vlim, unit) in enumerate(zip(axs, keys, kwargs['vlims'], kwargs['units'])):
        ...
        ax.tick_params(direction='in', length=10, width=2, colors='k')
